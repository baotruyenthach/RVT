...
class MVT(nnx.Module):
    ...
    def render(self, pc, img_feat, img_aug, mvt1_or_mvt2, dyn_cam_info):
        ...
        for _pc, _img_feat, _dyn_cam_info in zip(pc, img_feat, dyn_cam_info_itr):
            _pc = torch.tensor(np.asarray(_pc), dtype=torch.float32, device=self.renderer.device) \
                if not isinstance(_pc, torch.Tensor) else _pc.to(self.renderer.device)
            _img_feat = torch.tensor(np.asarray(_img_feat), dtype=torch.float32, device=self.renderer.device) \
                        if not isinstance(_img_feat, torch.Tensor) else _img_feat.to(self.renderer.device)

            # ---- handle add_corr ----
            if mvt.add_corr:
                if mvt.norm_corr:
                    max_pc = torch.tensor(1.0, device=_pc.device) if len(_pc) == 0 else torch.max(torch.abs(_pc))
                    concat_feat = torch.cat((_pc / max_pc, _img_feat), dim=-1)
                else:
                    concat_feat = torch.cat((_pc, _img_feat), dim=-1)
            else:
                concat_feat = _img_feat
        ...
        img = torch.cat(img, dim=0)                      # (bs, num_img, H, W, C)
        img = img.permute(0, 1, 4, 2, 3).contiguous()     # (bs, num_img, C, H, W)

        # Store image for visualization
        with torch.no_grad():
            if mvt.add_corr:
                mvt.img = img[:, :, 3:].detach().clone()
            else:
                mvt.img = img.detach().clone()

        # ---- Add pixel location if needed ----
        if mvt.add_pixel_loc:
            pixel_loc = jnp.transpose(mvt.pixel_loc, (1, 0, 2, 3))  # (num_img, 3, H, W)
            pixel_loc = jnp.broadcast_to(pixel_loc[None], (img.shape[0], *pixel_loc.shape))  # (bs, num_img, 3, H, W)
            pixel_loc = torch.tensor(np.asarray(pixel_loc), dtype=torch.float32, device=img.device)
            img = torch.cat((img, pixel_loc), dim=2)

        # ---- Add noise augmentation if requested ----
        if img_aug != 0:
            stdv = img_aug * torch.rand(1, device=img.device)
            noise = stdv * (2 * torch.rand_like(img) - 1)
            img = torch.clamp(img + noise, -1.0, 1.0)

        return jnp.array(img.cpu().numpy())  # Final output to JAX
