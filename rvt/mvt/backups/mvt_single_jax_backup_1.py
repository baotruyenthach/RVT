...
class MVT(nnx.Module):
        ...
        *,
        ...
        if cvx_up:
            ...
            self.up0 = Conv2DUpsampleBlock(
                ...
                norm=None, activation=activation, out_size=img_size,
                rngs=rngs,
            )
    ...
    def forward(
        img, proprio, lang_emb, wpt_local, rot_x_y,
        model,  # assume model is the initialized Flax NNX MVT model instance
    ):
        bs, num_img, img_feat_dim, h, w = img.shape
        num_pat_img = h // model.img_patch_size
        assert num_img == model.num_img
        assert h == w == model.img_size

        img = jnp.reshape(img, (bs * num_img, img_feat_dim, h, w))
        d0 = model.input_preprocess(img)

        ins = model.patchify(d0)
        ins = jnp.reshape(ins, (bs, num_img, model.im_channels, num_pat_img, num_pat_img))
        ins = jnp.transpose(ins, (0, 2, 1, 3, 4))  # (bs, im_channels, num_img, h', w')

        if model.add_proprio:
            p = model.proprio_preprocess(proprio)
            p = p[:, :, None, None, None]
            p = jnp.broadcast_to(p, (bs, model.im_channels, num_img, num_pat_img, num_pat_img))
        ...
        if model.pe_fix:
            ins += model.pos_encoding

        num_lang_tok = 0
        if model.add_lang:
            l = model.lang_preprocess(jnp.reshape(lang_emb, (bs * model.lang_max_seq_len, model.lang_emb_dim)))
            l = jnp.reshape(l, (bs, model.lang_max_seq_len, -1))
        ...
        x = model.fc_bef_attn(ins)
        if model.self_cross_ver == 0:
            for self_attn, self_ff in model.layers:
                x = self_attn(x) + x
                x = self_ff(x) + x
        elif model.self_cross_ver == 1:
            lx, imgx = x[:, :num_lang_tok], x[:, num_lang_tok:]
            imgx = jnp.reshape(imgx, (bs * num_img, num_pat_img * num_pat_img, -1))
            for self_attn, self_ff in model.layers[: len(model.layers) // 2]:
        ...
        x = jnp.reshape(x, (bs * model.num_img, model.input_dim_before_seq, num_pat_img, num_pat_img))

        if model.cvx_up:
            trans = model.up0(x)
            trans = jnp.reshape(trans, (bs, model.num_img, h, w))
        else:
            u0 = model.up0(x)
            if model.inp_pre_con:
                u0 = jnp.concatenate([u0, d0], axis=1)
            u = model.final(u0)
            trans = model.trans_decoder(u)
            trans = jnp.reshape(trans, (bs, model.num_img, h, w))

        out = {"trans": trans}

        if not model.no_feat and model.feat_ver == 0:
            hm = jnp.reshape(trans, (bs, model.num_img, h * w))
            hm = softmax(hm, axis=-1)
            hm = jnp.reshape(hm, (bs * model.num_img, 1, h, w))

            if model.cvx_up:
                _hm = conv_general_dilated_patches(
                    hm, (model.img_patch_size, model.img_patch_size),
                    window_strides=(model.img_patch_size, model.img_patch_size),
                    padding="VALID"
                )
                _hm = jnp.mean(_hm, axis=-1)
                _hm = jnp.reshape(_hm, (bs * model.num_img, 1, num_pat_img, num_pat_img))
                _u = x
            else:
                _hm = hm
                _u = u
...
def test_mvt_constructor():
    import jax
    import jax.numpy as jnp
    from types import SimpleNamespace
    import flax.nnx as nnx

    # Dummy renderer mock with required attribute
    class DummyRenderer:
    ...
    model = MVT(
        ...
        rot_ver=1,
        num_rot=4,
        renderer=DummyRenderer(num_img=3),
        renderer_device="cuda:0",
        no_feat=False,
        rngs=rng,
    )
    ...
    assert model.lang_emb_dim == 64
    assert model.pos_encoding.value.shape == (1, 3 * (64 // 8)**2, 64)  # [1, num_pe_token, dim]
    assert hasattr(model, "input_preprocess")
    assert hasattr(model, "patchify")
    assert hasattr(model, "fc_bef_attn")
    assert hasattr(model, "up0")

    # Optional: test one layer output shape
    x = jnp.ones((2, 3 + 3 + 1 + 8, 64, 64))  # num_channels: 8+3(corr)+3(pixel)+1(depth)
    out = model.input_preprocess(x)
    assert out.shape == (2, 32, 64, 64), f"input_preprocess output shape mismatch: {out.shape}"

    print("MVT constructor test passed.")

if __name__ == "__main__":
    test_mvt_constructor()