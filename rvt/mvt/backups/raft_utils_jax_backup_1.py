...
class ConvexUpSample(nnx.Module):
    ...
    def __call__(self, x: jax.Array) -> jax.Array:
        ...
        out = relu(self.net_out_conv1(x))
        out = relu(self.net_out_conv2(out))
        ...
        # (B, H, W, out_dim * K*K) → reshape and permute
        ...
        out = out.transpose(0, 2, 3, 1)  # Convert NCHW → NHWC
        return out


def test_convex_upsample():
    rng = nnx.Rngs(jax.random.key(0))
    model = ConvexUpSample(in_dim=32, out_dim=1, up_ratio=2, rngs=rng)
    x = jnp.ones((2, 8, 8, 32))  # ✅ Correct: NHWC
    out = model(x)
    assert out.shape == (2, 16, 16, 1), f"Output shape mismatch: {out.shape}"
    print("✅ ConvexUpSample test passed.")
